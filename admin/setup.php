...
/**
 * \file    libeufinconnector/admin/setup.php
 * \ingroup libeufinconnector
 * \brief   LibEuFinConnector setup page.
 */
...
/** @var HookManager $hookmanager */
$hookmanager->initHooks(array('libeufinconnectorsetup', 'globalsetup'));
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/libeufinconnector/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=libeufinconnector-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'LIBEUFINCONNECTOR_'.strtoupper($tmpobjectkey)."_ADDON";
...
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "libeufinconnector@libeufinconnector");
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'LIBEUFINCONNECTOR_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'LIBEUFINCONNECTOR_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
